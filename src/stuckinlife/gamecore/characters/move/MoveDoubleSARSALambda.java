...
import stuckinlife.gamecore.learning.DoubleSARSA;
import utils.Utils;
...
public class MoveDoubleSARSALambda extends MoveController {
    ...
    private final int ATTACK_READY = 2;
    private final double LENGTH;
    private final int[] DIMENSIONS = {NUM_ANGLES, NUM_DISTANCES, NUM_DIRECTIONS, ATTACK_READY};
    private final boolean LEARNING_ON;

    private DoubleSARSA Q_LEARNING;
    ...
    public MoveDoubleSARSALambda(World world, String name, int moveReset, double aiViewRange, boolean learningOn) {
        super(world);
        MOVE_DIR_RESET_TIME = moveReset;
        LENGTH = aiViewRange / NUM_DISTANCES;
        FILE_NAME = "ai" + SEPARATOR + "move" + SEPARATOR + name + "DoubleSARSAlambda";
        DoubleSARSA model = getModel();
        Q_LEARNING = model;
        LEARNING_ON = learningOn;
        lastState = new int[DIMENSIONS.length];
    }

    public DoubleSARSA getModel() {
        File qLearningA = new File(FILE_NAME);
        DoubleSARSA model = null;
        try {
            if (qLearningA.createNewFile()) {
                model = new DoubleSARSA(DIMENSIONS, MOVE_CHOICES, LEARNING_RATE, DISCOUNT_FACTOR);
            } else {
                model = readModel();
            }
            saveModel(model);
    ...
    private DoubleSARSA readModel() {
        try (FileInputStream in = new FileInputStream(FILE_NAME);
             ObjectInputStream s = new ObjectInputStream(in)) {
            double[][][] qTable = (double[][][]) s.readObject();
            DoubleSARSA model = new DoubleSARSA(DIMENSIONS, MOVE_CHOICES, LEARNING_RATE, DISCOUNT_FACTOR);
    ...
    public void saveModel(DoubleSARSA model) {
            ...
            if (moveDirResetCounter == 0) {
                ...
                if (Utils.distanceEuclidean(closestEnemy.getX(), closestEnemy.getY(), owner.getX(), owner.getY()) > LENGTH * NUM_DISTANCES) {
                    ...
                    if (LEARNING_ON && Math.random() < EPSILON) {
                        ...
                        //System.out.println(FILE_NAME + " " + Arrays.toString(actions));
                        for (int i = 0; i < 4; i++) {
                            if (actions[i] > highest && wallDistances[i] > owner.getMoveSpeed() / 2) {
                                highest = actions[i];
                                action = i;
                            }
                        }
                    ...
                    if (LEARNING_ON) {
                        ...
                        if (!infoTracing.isEmpty()) {
                            ...
                            for (Info info : infoTracing) {
                                ...
                                for (int i = replay.size() - 2; i >= 0; i--) {
                                    ...
                                    Q_LEARNING.update(currentStateAction.STATE, currentStateAction.ACTION, reward, nextStateAction.STATE, lastAction);
                                    lastAction = currentStateAction.ACTION;
                                    reward *= REWARD_DECAY;
                                }
                            }
                            infoTracing.clear();
                        ...
                        if (!info.isEmpty()) {
                            ...
                            int lastAction = action;
                            for (Info info : info) {
                                Q_LEARNING.update(info.STATE, info.ACTION, info.REWARD, state, lastAction);
    ...
    public int[] getState(Char enemy) {
        ...
        state[3] = owner.getAttackCooldown() == 0 ? 1 : 0;
